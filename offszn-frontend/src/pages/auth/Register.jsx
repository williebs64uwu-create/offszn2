...
const Register = () => {
  ...
  const onSubmit = async (data) => {
    ...
    try {
      ...
      // Redirect to verification instead of onboarding directly
      navigate('/auth/verify-email' + (redirectToFromURL ? `?redirect=${redirectToFromURL}` : ''));
    } catch (error) {
  ...
  const handleGoogleRegister = async () => {
    try {
      await signInWithGoogle(redirectToFromURL || '/welcome');
    } catch (error) {
  ...
  return (
    <>
      ...
      <div className="text-center mb-8">
        <h1 className="text-white text-2xl font-semibold mb-2 font-display">Crear una cuenta</h1>
      ...
      <p className="text-center text-zinc-400 text-sm mt-6">
        ...
        <Link to="/auth/login" className="text-white hover:underline font-medium">Inicia sesión</Link>
      </p>
    </>
  );
};
